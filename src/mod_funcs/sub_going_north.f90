PURE SUBROUTINE sub_going_north(ixOld, iyOld, elev, z, ixNew, iyNew)
    ...
    IF(ixOld /= 1_INT64 .AND. iyOld /= 1_INT64 .AND. elev(ixOld, iyOld - 1_INT64) >= z .AND. elev(ixOld - 1_INT64, iyOld - 1_INT64) >= z)THEN
        CALL sub_go_west(ixOld, iyOld, ixNew, iyNew)
        RETURN
    END IF

    ! Check if we can go North ...
    IF(iyOld /= 1_INT64 .AND. elev(ixOld, iyOld - 1_INT64) >= z)THEN
